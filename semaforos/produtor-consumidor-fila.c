...
Descrição: Crie um programa com duas threads: uma thread produtora que adiciona itens a uma fila e uma thread consumidora que remove itens da fila. A fila deve ter uma capacidade limitada e a sincronização deve ser usada para  garantir que a thread consumidora não tente remover itens de uma fila vazia e  que a thread produtora não tente adicionar itens a uma fila cheia
*/
